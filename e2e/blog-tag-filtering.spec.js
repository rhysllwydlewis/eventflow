...
test.describe('Blog Tag Filtering', () => {
  ...
  test('should have proper styling on filter buttons', async ({ page }) => {
    ...
    const bgColor = await allButton.evaluate(el => window.getComputedStyle(el).backgroundColor);
    expect(bgColor).toBeTruthy();

    const borderRadius = await allButton.evaluate(el => window.getComputedStyle(el).borderRadius);
  ...
  test('should handle hover state on filter buttons', async ({ page }) => {
    ...
    if (filters.length > 0) {
      ...
      const hoverBg = await filter.evaluate(el => window.getComputedStyle(el).backgroundColor);
  ...
  test('should have proper tag badges layout', async ({ page }) => {
    ...
    const display = await tagsContainer.evaluate(el => window.getComputedStyle(el).display);
    expect(display).toBe('flex');

    const flexWrap = await tagsContainer.evaluate(el => window.getComputedStyle(el).flexWrap);
