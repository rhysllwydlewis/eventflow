...
test.describe('Homepage Performance - Critical CSS', () => {
  test('should have inline critical CSS in <head>', async ({ page }) => {
    ...
    const criticalCSSFound = inlineStyles.some(style => 
      style.includes('.ef-header') && 
      style.includes('.container') &&
      style.includes('.hero')
    );
...
test.describe('Homepage Layout - Responsive Testing', () => {
  Object.entries(VIEWPORTS).forEach(([name, viewport]) => {
    test(`should render without horizontal scroll at ${name} (${viewport.width}px)`, async ({ page }) => {
      ...
      console.log(`✓ ${name}: No horizontal scroll (body: ${bodyWidth}px, viewport: ${viewportWidth}px)`);
    });
