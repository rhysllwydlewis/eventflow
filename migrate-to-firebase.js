...
async function migrateCollection(collectionName, filePath) {
  ...
  if (options.dryRun) {
    ...
    localData.slice(0, 3).forEach(item => {
      console.log(
        `      - ${item.id || 'no-id'}: ${item.name || item.title || item.email || 'unnamed'}`
      );
    });
  ...
  for (const item of localData) {
    ...
    if (!docId) {
      console.log(
        `   ⚠️  Skipping item without ID. Available fields: ${Object.keys(item).join(', ')}`
      );
...
async function migrate() {
  ...
  if (!isFirebaseAvailable()) {
    ...
    console.error(
      'Please set FIREBASE_PROJECT_ID or FIREBASE_SERVICE_ACCOUNT_KEY environment variable.'
    );
    console.error('\nExample:');
