...
class ExportUtility {
  ...
  async exportComparisonToPDF(suppliers, filename = 'supplier-comparison.pdf') {
    ...
    suppliers.forEach((supplier, index) => {
      ...
      y += 6;
      doc.text(`Starting Price: £${supplier.startingPrice?.toLocaleString() || 'Contact for quote'}`, 25, y);
      y += 6;
      doc.text(`Rating: ${supplier.rating?.toFixed(1) || 'N/A'} (${supplier.reviewCount || 0} reviews)`, 25, y);
      y += 6;
      doc.text(`Capacity: ${supplier.minGuests || 0} - ${supplier.maxGuests || 'Unlimited'} guests`, 25, y);
      y += 6;
