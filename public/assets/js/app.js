    ...
    if (aiRun && aiOut) {
      aiRun.addEventListener('click', async () => {
        ...
        try {
          ...
          if (checklist.length) {
            html +=
              '<h4>Suggested checklist</h4><ul>' +
              checklist.map(t => '<li>' + escapeHtml(String(t)) + '</li>').join('') +
              '</ul>';
          }
          if (timeline.length) {
            html +=
              '<h4>Sample timeline</h4><ul>' +
              timeline
                .map(
                  row =>
                    '<li><strong>' +
                    escapeHtml(String(row.time || '')) +
                    '</strong> – ' +
                    escapeHtml(String(row.item || '')) +
                    (row.owner
          ...
          if (suppliers.length) {
            html +=
              '<h4>Supplier tips</h4><ul>' +
              suppliers
                .map(
                  row =>
                    '<li><strong>' +
                    escapeHtml(String(row.category || 'Supplier')) +
                    ':</strong> ' +
                    escapeHtml(String(row.suggestion || '')) +
                    '</li>'
                )
                .join('') +
              '</ul>';
          ...
          if (budget.length) {
            html +=
              '<h4>Budget breakdown</h4><ul>' +
              budget
                .map(
                  row =>
                    '<li><strong>' +
                    escapeHtml(String(row.item || 'Item')) +
                    ':</strong> ' +
                    escapeHtml(String(row.estimate || '')) +
                    '</li>'
                )
                .join('') +
              '</ul>';
          ...
          if (styleIdeas.length) {
            html +=
              '<h4>Style &amp; theme ideas</h4><ul>' +
              styleIdeas.map(t => '<li>' + escapeHtml(String(t)) + '</li>').join('') +
              '</ul>';
          }
          if (messages.length) {
            html +=
              '<h4>Messages to suppliers</h4><ul>' +
              messages.map(t => '<li>' + escapeHtml(String(t)) + '</li>').join('') +
              '</ul>';
          }
...
function efMaybeShowOnboarding(page) {
  ...
  box.innerHTML =
    '<h2 class="h4">Welcome to EventFlow</h2>' +
    body +
    ...
    tbody.innerHTML = items
      .map(function (u) {
        return (
          ...
          '</td>' +
          '<td>' +
          efFormatDate(u.createdAt) +
          '</td>' +
          '<td>' +
          (u.lastLoginAt ? efFormatDate(u.lastLoginAt) : 'Never') +
          '</td>' +
          '</tr>'
        );
      })
...
function efInitVenueMap() {
  ...
  function updateForQuery(q) {
    ...
    const url =
      ...
      encodeURIComponent('wedding venues near ' + q) +
      '&output=embed';
    mapFrame.src = url;
