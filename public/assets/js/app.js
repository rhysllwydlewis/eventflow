...
(function(){
  ...
  function showNetworkError(message) {
    try {
      ...
      efErrorBanner.textContent = message || 'Could not reach EventFlow. Please check your connection and try again.';
      efErrorBanner.style.opacity = '1';
      clearTimeout(efErrorBanner._hideTimer);
      efErrorBanner._hideTimer = setTimeout(function(){
        efErrorBanner.style.opacity = '0';
      }, 5000);
    } catch (_) {}
  }
  ...
  if (typeof window !== 'undefined' && window.fetch) {
    ...
    window.fetch = function(){
      return realFetch.apply(this, arguments).catch(function(err){
        showNetworkError();
        throw err;
      });
    };
...
async function initResults(){
  ...
  if (qp) {
    filters.q = qp;
    if (filterQueryEl) filterQueryEl.value = qp;
  }
  ...
  if (contextEl) {
    try {
      ...
      if (raw) {
        ...
        if (typeof data.guests === 'number' && data.guests > 0) bits.push(data.guests + ' guests');
        if (data.budget) bits.push(data.budget);
        contextEl.textContent = bits.length
          ? 'Based on your last event: ' + bits.join(' • ')
          : '';
      }
    } catch (_e) {
      // ignore
  ...
  async function render(){
    ...
    if (!items.length) {
      container.innerHTML = '<div class="card"><p>No suppliers match your search yet. Try clearing filters, searching by town or city, or starting again from the <a href="/start.html">Start page</a>.</p></div>';
      return;
    }
  ...
  if (filterCategoryEl) {
    filterCategoryEl.addEventListener('change', e=>{ filters.category=e.target.value||''; render(); });
  }
  if (filterPriceEl) {
    filterPriceEl.addEventListener('change', e=>{ filters.price=e.target.value||''; render(); });
  }
  if (filterQueryEl) {
    filterQueryEl.addEventListener('input', e=>{ filters.q=e.target.value||''; render(); });
  }
  ...
  document.querySelectorAll('[data-quick-category]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      if (filterCategoryEl) filterCategoryEl.value = cat;
      render();
    });
  });
...
async function initSupplier(){
  ...
  if(!r.ok){
    ...
    if(c) c.innerHTML='<div class="card"><p>Not found.</p></div>';
    return;
  }
  ...
  if(startThreadBtn){
    startThreadBtn.addEventListener('click', async ()=>{
      ...
      modal.querySelector('#send-thread').addEventListener('click', async ()=>{
        ...
        if(!r.ok){ alert((d && d.error) || 'Could not start conversation'); return; }
...
async function initPlan(){
  ...
  function loadLocalPlan(){
    try{
      ...
      if (!raw) return { version: 2, guests: [], tasks: [], timeline: [], notes: '' };
      const obj = JSON.parse(raw);
      return Object.assign({ version: 2, guests: [], tasks: [], timeline: [], notes: '' }, obj || {});
    }catch(_e){
  ...
  if (eventSummary) {
    try {
      ...
      if (raw) {
        ...
        eventSummary.textContent = bits.length ? bits.join(' · ') : 'Tell us about your event to see a quick summary here.';
      } else {
  ...
  if (container) {
    if (!items.length) {
      container.innerHTML = '<div class="card"><p>Your plan is currently empty. Add suppliers from the Suppliers page to build it.</p></div>';
    } else {
      container.innerHTML = items.map(s => supplierCard(s, user)).join('');
    }
  }
  ...
  try {
    ...
    if (aiRun && aiOut) {
      aiRun.addEventListener('click', async () => {
        let promptText = (aiInput && aiInput.value || '').trim();
        ...
        if (plan && typeof plan === 'object') {
          if (Array.isArray(plan.guests) && plan.guests.length) summaryBits.push(plan.guests.length + ' guests in the guest list');
          ...
          if (Array.isArray(plan.budgetItems) && plan.budgetItems.length) summaryBits.push(plan.budgetItems.length + ' budget lines');
        }
        ...
        try {
          ...
          if (!r.ok) {
            aiOut.innerHTML = '<p class="small">Sorry, something went wrong while generating suggestions.</p>';
            return;
          }
          ...
          if (!html) {
            html = '<p class="small">AI did not return any structured suggestions. Try adding a bit more detail to your description.</p>';
          }
...
// --- Budget tracker (manual, but seeded with supplier count) ---
  if (budgetEl) {
    ...
    let est = '';
    if (totalSuppliers === 0) {
      est = 'No suppliers added yet. Once you add venues, catering and more, you can track budget here.';
    } else if (totalSuppliers === 1) {
      est = 'You have 1 supplier in your plan. Use this section to keep track of quotes and payments.';
    } else {
    ...
    if (addBudget) {
      addBudget.addEventListener('click', () => {
        ...
        if (!label && !estimate) return;
        const row = { id: 'b_' + Date.now().toString(36), label, estimate, actual, notes: notesVal };
        if (!plan.budgetItems) plan.budgetItems = [];
  ...
  function renderGuests(){
    ...
    if (!guests.length) {
      host.innerHTML = '<p class="plan-list-empty">No guests added yet. Start by adding your VIPs (wedding party, parents, etc.).</p>';
      return;
    }
    host.innerHTML = `
      <div class="plan-list-block">
  ...
  function renderTimeline(){
    ...
    if (!timeline.length) {
      host.innerHTML = '<p class="plan-list-empty">No timeline items yet. Add key parts of the day below.</p>';
      return;
    }
    host.innerHTML = `
      <div class="plan-list-block">
  ...
  function updateProgress(){
    ...
    if (!wrap || !percentEl || !breakdownEl) return;
    ...
    let score = 0;
    let max = 4;
    if (hasSuppliers) score++;
    if (hasGuests) score++;
    if (hasTimeline) score++;
    if (totalTasks && doneTasks === totalTasks) score++;
  ...
  if (addTimeline) {
    addTimeline.addEventListener('click', ()=>{
      ...
      if (!plan.timeline) plan.timeline = [];
  ...
  if (notesEl) {
    if (plan.notes) notesEl.value = plan.notes;
    notesEl.addEventListener('input', ()=>{
      plan.notes = notesEl.value;
      saveLocalPlan(plan);
      if (status) status.textContent = 'Saved';
      if (cloud) cloud.textContent = 'Saved locally';
...
async function renderThreads(targetEl){
  ...
  try {
    ...
    const d = await r.json();
    const items = (d.items || []).slice().sort((a,b)=> (b.updatedAt||'').localeCompare(a.updatedAt||''));

    if (!items.length) {
      host.innerHTML = '<p class="small">No conversations yet. Once you contact suppliers, they’ll appear here.</p>';
      return;
    }
...
async function openThread(id){
  ...
  await load(true);
  efThreadPoll = setInterval(()=>{ load(false); }, 5000);
...
function efMaybeShowOnboarding(page){
  ...
  if(page === 'dash_customer'){
    body = '<p class="small">Here\'s what to do next:</p>' +
    ...
    body = '<p class="small">Quick tips:</p>' +
    ...
    body = '<p class="small">Quick overview:</p>' +
  ...
  box.innerHTML = '<h2 class="h4">Welcome to EventFlow</h2>' + body +
...
async function initDashCustomer(){ efMaybeShowOnboarding('dash_customer'); await renderThreads('threads-cust'); }
async function initDashSupplier(){ 
  ...
  async function loadSuppliers(){
    ...
    if(proRibbon){
      if(currentIsPro){
        ...
        proRibbon.innerHTML='<strong>You’re on EventFlow Pro.</strong> Your listing can appear higher in search and you have access to premium features as we roll them out.';
      }else{
        proRibbon.style.display='block';
        proRibbon.innerHTML='<strong>You’re on the free plan.</strong> Upgrade to EventFlow Pro to boost your visibility, unlock more packages and get priority support.';
      }
    }

    if(!supWrap) return;
    if(!items.length){
      supWrap.innerHTML='<div class="card"><p>You have not created a supplier profile yet.</p></div>';
      return;
    }
    supWrap.innerHTML=items.map(s=>`<div class="supplier-card card" style="margin-bottom:10px">
    ...
    items.forEach((s, idx)=>{
      ...
      if(label){
        label.textContent = score ? `Listing health: ${score}%` : 'Listing health: add photos and details to improve this listing.';
      }
    });
  ...
  async function loadPackages(){
    ...
    if (pkgForm) {
      ...
      if (pkgStatus) {
        if (atLimit) {
          pkgStatus.textContent = 'You have reached the package limit on the free plan. Upgrade to Pro to add more.';
        } else {
    ...
    pkgsWrap.innerHTML=items.map(p=>`<div class="card package-card">
      <img src="${p.image}" alt="${p.title} image">
      <div>
        <h3>${p.title}</h3>
  ...
  efSetupPhotoDropZone('sup-photo-drop','sup-photo-preview', function(dataUrl){
    var area = document.getElementById('sup-photos');
    if (!area) return;
    var current = (area.value || '').split(/\r?\n/).map(function(x){ return x.trim(); }).filter(Boolean);
    current.push(dataUrl);
    area.value = current.join('\n');
  });

  efSetupPhotoDropZone('pkg-photo-drop','pkg-photo-preview', function(dataUrl){
    var input = document.getElementById('pkg-image');
    if (!input) return;
    input.value = dataUrl;
  });
...
async function initAdmin(){ efMaybeShowOnboarding('admin');
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', async () => {
      if (!window.confirm('Reset demo data? This will clear demo users, suppliers and plans.')) return;
...
async function initAdminUsers(){
  ...
  try{
    var data = await fetchJSON('/api/admin/users');
    ...
    tbody.innerHTML = '<tr><td colspan="7" class="small">You must be signed in as an admin to view this page.</td></tr>';
  }
}

document.addEventListener('DOMContentLoaded', ()=>{
  const page = window.__EF_PAGE__ || (location.pathname.endsWith('admin-users.html')?'admin_users': location.pathname.endsWith('admin.html')?'admin': location.pathname.endsWith('auth.html')?'auth': location.pathname.endsWith('verify.html')?'verify': location.pathname.endsWith('dashboard-customer.html')?'dash_customer': location.pathname.endsWith('dashboard-supplier.html')?'dash_supplier': location.pathname.endsWith('suppliers.html')?'results': location.pathname.endsWith('supplier.html')?'supplier': location.pathname.endsWith('plan.html')?'plan': '');
  ...
  (async () => {
    try {
      ...
      if (!r.ok) { label.textContent = 'unknown'; return; }
      const data = await r.json();
  ...
  // Per-page setup
  if(page==='home') initHome && initHome();
  if(page==='results') initResults && initResults();
  if(page==='supplier') initSupplier && initSupplier();
  if(page==='plan') initPlan && initPlan();
  if(page==='dash_customer') renderThreads && renderThreads('threads-cust');
  if(page==='dash_supplier') initDashSupplier && initDashSupplier();
  if(page==='admin') initAdmin && initAdmin();
    ...
    try{
      ...
      if(existing && loginStatus){
        ...
        setTimeout(()=> {
          ...
          else if(existing.role === 'supplier') location.href = '/dashboard-supplier.html';
  ...
  if(forgotLink && loginEmail){
    forgotLink.addEventListener('click', async (e)=>{
      ...
      try{
        ...
        if(loginStatus) loginStatus.textContent = 'Something went wrong. Please try again in a moment.';
      }
    });
  }
  ...
  if(loginForm && loginEmail && loginPassword){
    ...
    loginForm.addEventListener('submit', async (e)=>{
      ...
      try{
        ...
        if(!r.ok){
          if(loginStatus) loginStatus.textContent = data.error || 'Could not sign in. Please check your details and try again.';
        }else{
          if(loginStatus) loginStatus.textContent = 'Signed in. Redirecting…';
          const user = data.user || {};
          try { localStorage.setItem('eventflow_onboarding_new', '1'); } catch (_e) {}
          if(user.role === 'admin') location.href = '/admin.html';
          else if(user.role === 'supplier') location.href = '/dashboard-supplier.html';
          else location.href = '/dashboard-customer.html';
        }
      }catch(err){
        if(loginStatus) loginStatus.textContent = 'Network error – please try again.';
