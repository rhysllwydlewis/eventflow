...
router.get('/:id/export', authRequired, async (req, res) => {
  try {
    ...
    // Plan Details
    doc.fontSize(18).font('Helvetica-Bold').text(plan.name || 'Untitled Event', 100, 120);
    doc.moveDown(0.5);
    ...
    if (plan.budgetItems && plan.budgetItems.length > 0) {
      ...
      plan.budgetItems.forEach((item, index) => {
        ...
        doc.text(`${index + 1}. ${item.category || 'Other'} - ${item.item || 'Unnamed'}`, 100, doc.y);
        doc.text(`   Estimated: £${(item.estimated || 0).toFixed(2)} | Actual: £${(item.actual || 0).toFixed(2)} | Paid: £${(item.paid || 0).toFixed(2)}`, 100, doc.y + 15);
        if (item.notes) {
    ...
    if (plan.timeline && plan.timeline.length > 0) {
      ...
      plan.timeline.forEach((item, index) => {
        ...
        doc.text(`${item.time || ''} - ${item.activity || item.name || `Activity ${index + 1}`}`, 100, doc.y);
        if (item.notes) {
    ...
    doc.fontSize(8).font('Helvetica').text(
      `Generated by EventFlow on ${new Date().toLocaleString()}`,
      100,
      doc.page.height - 50,
      { align: 'center' }
    );
