 ...
 */
router.post('/backup/create', authRequired, roleRequired('admin'), csrfProtection, async (req, res) => {
  try {
    ...
    const collections = [
      'users', 'suppliers', 'packages', 'categories', 'plans', 'notes',
      'messages', 'threads', 'events', 'reviews', 'reports', 'audit_logs',
      'search_history', 'photos', 'payments', 'settings', 'badges',
      'marketplace_listings', 'tickets', 'newsletterSubscribers', 'savedItems'
    ];
 ...
 * Body: { filename }
 */
router.post('/backup/restore', authRequired, roleRequired('admin'), csrfProtection, async (req, res) => {
  try {
 ...
 */
router.post('/packages/:id/duplicate', authRequired, roleRequired('admin'), csrfProtection, async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/pexels/test', authRequired, roleRequired('admin'), async (req, res) => {
  try {
    ...
    // Test API by fetching sample images
    ...
    res.json({
      ...
      sampleImages: data.photos ? data.photos.slice(0, 3).map(p => ({
        id: p.id,
        url: p.src.medium,
...
router.post('/users/:id/impersonate', authRequired, roleRequired('admin'), csrfProtection, async (req, res) => {
  try {
    ...
    if (req.session.originalUser) {
      return res.status(400).json({ error: 'Already impersonating a user. Stop current session first.' });
    }
